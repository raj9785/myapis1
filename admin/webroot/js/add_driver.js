...
//        }
        ...
        if ($.trim($("#license_no").val()) != '') {
            ...
            if ($("#license_no").val().length < 6 || $("#license_no").val().length > 21) {
                $("#license_no-error").html("License Number number can be of 12 digits only ");
                $("#license_no").parent('div').addClass('has-error');
                $("#license_no").focus();
                return false;
            } else {
            ...
            if (!($.trim($("#license_no").val())).match(/^(?=.*[a-zA-Z \s])(?=.*[0-9])/)) {
                $("#license_no-error").html("License Number Only aphanumeric value is allowed");
                $("#license_no").parent('div').addClass('has-error');
                $("#license_no").focus();
                $("#card-error").html("");
                return false;
            ...
            var pattern = /\s/;
            if (document.getElementById("license_no").value.match(pattern))
            {
                //alert("Whitespaces are not allowed");
                $("#license_no-error").html("Whitespaces are not allowed in License Number");
                $("#license_no").parent('div').addClass('has-error');
                $("#license_no").focus();
                $("#card-error").html("");
                return false;
