...
// Open editbar
function showEditBar(imgSrc, imgStyle, imgID, imgName,folderName) {
    ...
    $("#editbarDelete").attr("onclick","deleteImg('" + imgName + "', '" + imgID + "','" + folderName + "');");
...
function drop(e,unique_id) {
    ...
    if(file && file.type.match("image/*")) {
        ...
        xhr.open('POST', 'imgupload.php?image_path='+unique_id);
        xhr.send(formdata);
...
// delete image
function deleteImg(src, imgid,folderName) {
    $.ajax({
       method: "GET",
        url: "imgdelete.php",
        data: {
            img: src,
            unique_id: folderName,
        }
    }).done(function() {
...
// reload images
function reloadImages(folderName) {
    $.ajaxSetup({cache:false});
    $('#files').load('function.php?f=loadImages&=unique_id'.folderName, function(response, status, xhr) {
         $("img.lazy").lazyload();
     });
}
